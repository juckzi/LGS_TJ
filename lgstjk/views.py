...
def index(request):
    ...
    return render(request,
        'lgstjk/tjgz.html',context)

def csvgen(request):
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="老干所统计收费.csv'
    ...
    if request.is_ajax():
        ...
        if dept == '全所科室':
            dept = ''
        tjgz = lgstj('%s-%s-%s'%(startyear,startmonth,startday),'%s-%s-%s'%(endyear,endmonth,endday),mzorzy=checked,dept_name=dept)
...
def tjgzfreshtable(request):
    if request.is_ajax():
        ...
        return render(request,
                          'lgstjk/tjgzFreshtable.html',context)
